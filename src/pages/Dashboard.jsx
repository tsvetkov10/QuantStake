...
const BetCardPopup = ({ rawBet, sym }) => {
  ...
  const statusInfo = rawBet.status === 'Won' ? { color: '#10b981', label: 'SETTLED • WIN', shadow: '0 0 12px rgba(16,185,129,0.4)' } : 
                     ...
                     { color: '#9d8df0', label: 'PENDING', shadow: 'none' };
    ...
    >
      ...
      <div className="flex-col gap-3">
        <div className="flex justify-between items-center">
          <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#9d8df0', background: 'rgba(255, 255, 255, 0.05)', padding: '0.2rem 0.6rem', borderRadius: '20px', textTransform: 'uppercase' }}>
            {rawBet.sport || 'OTHER'}
          </span>
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', userSelect: 'none' }}>
      <svg width="100%" height="100%" viewBox={`0 0 ${viewWidth} ${viewHeight}`} preserveAspectRatio="none" style={{ overflow: 'visible' }}>
        ...
        {yMin < 0 && yMax > 0 && (() => {
          ...
          return <line x1="0" y1={zeroY} x2={viewWidth} y2={zeroY} stroke="rgba(72, 51, 181, 0.3)" strokeDasharray="4 4" strokeWidth="1.5" />;
        })()}
        ...
        {data.map((item, index) => {
          ...
          const candleColor = isBull ? '#ff8c00' : '#4833b5';
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{
        ...
        <div style={{ fontWeight: 'bold', color: '#9d8df0', fontSize: '0.95rem' }}>
...
export default function Dashboard({ session, profile }) {
  ...
  const pieColors = ['#9d8df0', '#ff3366', '#FFD700', '#00ffaa', '#4833b5', '#ff8c00', '#10b981'];
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'var(--bg-modal)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', zIndex: 100000, marginTop: '1.5rem' }}>
            ...
            <button 
              ...
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(72, 51, 181, 0.1)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(72, 51, 181, 0.03)'; }}
            >
              ...
              <div style={{ display: 'flex', gap: '3px', background: 'rgba(255,255,255,0.04)', padding: '3px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 'bold',
                    border: 'none',
                    background: chartType === 'candlestick' ? 'linear-gradient(135deg, #ff8c00, #4833b5)' : 'transparent',
                ...
                <AreaChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorBalanceGraph" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={netProfit >= 0 ? "#9d8df0" : "#ef4444"} stopOpacity={0.4}/>
                  ...
                  <Tooltip content={<CustomTooltip sym={sym} />} cursor={{ stroke: 'rgba(72, 51, 181, 0.4)', strokeDasharray: '3 3' }} />
                  <Area 
                    yAxisId="left" 
                    type="monotone" 
                    dataKey="balance" 
                    stroke={netProfit >= 0 ? "#9d8df0" : "#ef4444"} 
                    strokeWidth={3} 
                    fill="url(#colorBalanceGraph)" 
                    dot={{ r: 4, fill: '#0d0f17', stroke: netProfit >= 0 ? "#9d8df0" : "#ef4444", strokeWidth: 2 }} 
                    activeDot={{ r: 7, fill: netProfit >= 0 ? "#9d8df0" : "#ef4444", stroke: '#ffffff', strokeWidth: 2 }} 
                  />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
